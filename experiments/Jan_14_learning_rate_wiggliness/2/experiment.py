...
N_meta_iter = 20
all_log_alpha_0 = np.linspace(-1, 2.5, N_meta_iter)
...
def plot():
    ...
    ax.plot(all_log_alpha_0, losses, 'o-', label="loss")
    #ax.set_ylim([-0, 3])
    ...
    ax.plot(all_log_alpha_0, d_losses, 'o-', label="Gradient of loss")
    #ax.set_title("Grad loss vs step size")
    #ax.set_ylim([-0.5, 0.5])
    ax.set_ylabel("Training loss gradient")
    ax.set_xlabel(xlabel)
    #ax.legend(loc=2)
