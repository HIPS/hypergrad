...
\footnote{A note on terminology: since this paper is about hyperparameters we
  need a term to unambiguously denote the other sort of parameter, the
  ``parameter-that-is-just-a-parameter-not-a-hyperparameter''. We settled on
  ``\primal''.}  The problem with taking gradients with resepct to
hyperparameters is that computing the hyperloss (validation loss at optimized
\primal parameters, say) requires a large number of optimization steps, which
makes naive reverse mode differentiation unfeasible from a memory
perspective. Section \ref{hypergradients} describes this problem and proposes a
solution. This is our main technical contribution.

Gaining access to gradients with respect to hyperparamters opens up an exciting
field of research possibilities. Instead of straining to eliminate
hyperparameters from out models, we can embrace them, and hyperparameterize our
models as fully as they deserve. Just as having a high-dimensional \primal
parameterization gives a flexible model, having a high-dimensional
hyperparameterization gives flexibility over model classes. Section
\ref{experiments} begins to explore these ideas.
...
%From an optimization point of view, optimizing hyperparameters using stochastic hyper-gradients is no different from optimizing \primal{} parameters using stochastic gradients.
...
Likewise, the use of hyper-gradients also has several apparent difficulties that need to be addressed before it becomes practical.
...
Learning hyperparameters whose gradient itself depends on hundreds of iterations of gradient descent raises some of the same issues.
...
%Several recent papers have attempted to set training hyper-parameters automatically, or lessen sensitivity to their exact values~\cite{schaul2012no, Adasecant14, Hotswap14}.
