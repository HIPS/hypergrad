...
that control model complexity, such as L1 and L2 penalties, or parameters that
...
from a memory perspective. This is the central problem that we solve in this
paper.

Once we have access to gradients with respect to hyperparamters we open up a
garden of delights...

\footnote{Non-standard terminolgy. We use ``\primal'' for the inner
  optimization/parameters and ``hyper'' or ``meta'' for the outer
  optimization/parameters to avoid any ambiguity.}
...
Outline of paper: first intoduce our algorithm for reversible learning.
...
\begin{center}
\begin{tabular}{cccc}
\hspace{-3mm}&\hspace{-3mm}Independent &\hspace{-4mm} First layer &\hspace{-5mm} Learned \\
\hspace{-3mm}&\hspace{-3mm}networks    &\hspace{-4mm} shared      &\hspace{-5mm} sharing \\
\hspace{-3mm}\rotatebox{90}{\qquad First layer \qquad Second layer \quad Third layer} & 
...
TODO: therefore no faster than finite differences
