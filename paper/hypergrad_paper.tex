...
Here we compute exact gradients of cross-validation performance with respect to all hyperparameters by chaining derivatives backwards through the \emph{entire training procedure}.
...
We compute hyperparameter gradients by exactly reversing the dynamics of stochastic gradient descent with momentum. %, storing only the information lost due to finite precision arithmetic.
\end{abstract} 
...
Hessian-vector product, but these can be computed exactly by apply RMD to the
...
Every time we apply step XX to reduce the velocity, we
...
\footnote{We assume fixed-point representation to simplify the discussion (and
  the implenetation). Floating point representation doesn't fix the problem, it
...
$\decay = 1$, recovers the leapfrog integrator~\citep{leapfrog1995}, a perfectly
sensible (and reversible) set of dynamics but not one that converges.

This problem is quite a deep one: optimization necessarily discards information.
Ideally, optimization maps all initializations to the same minimum,
...
It is interesting to consider the anology with physical dynamics. The $\decay$
...
reversible. But we require dissipation in order for our system to equilibrate
to the energy minimum we care about.
...
section tries to achieve.

\section{Optimal storage of discarded entropy}
\label{sec:reversible computation}

We would like to store the information discarded each time the momentum decay
operation (XX) is applied. If we had $\decay = 0.5$ then we could just store the
single bit that falls off at each iteration and if $\decay = 0.25$ we could
store two bits. But we want fairly fine-grained control over $\decay$ so we want
some way of storing the required bits when we multiply by, say, $\decay = 0.9$.
And if $\decay > 0.5$ then we would somehow like to store less than one
bit per iteration on average. Here we give a procedure which achieves exactly
this.

We represent the velocity $v$ and position $x$ vectors with 64-bit integers. (Of
course there is an implied radix point so that this is actually a fixed-point
representation of the reals.) And we represent $\decay$ as a rational number,
$n/d$. When we divide $v$ by $d$ we use integer division. In order to be able to
reverse the process we just need to store the remainder, $v \% s$ in some
``information buffer'', $B$. If $B$ was an integer and $n = 2$, the remainder $r$ would just be a single
bit, and we could store it in $B$ by left-shifting $B$'s bits and adding $r$. 
For arbitrary $n$, we can just do the base-$n$ analogue of this operation: multiply 
$B$ by $n$ and add $r$. Eventually, $B$ will. We need a way to either detect
this, store the bits, and start a fresh integer, or else we can just use an
arbitary size integer that grows as needed. (Python's ``long'' integer type
...
extra, since integer division will bring us make to exactly the same point
anyway. But the procedure as it stand would use three bits with $\decay = 7/8$
whereas it should store less than one (hopefully $\log_2 (8/7)$). Here's the
trick we can use. When we multiply $v$ by $n$, there is an opportunity to add a
nonnegative integer smaller than $n$ to the result without affecting the reverse
process (intger division by $n$). We can get such an integer from the
information buffer by dividing it by $n$ and recording $B \% n$. We are using
the velocity $v$ as an information buffer itself! Algorithm
\ref{alg:reversible-mult} illustrates the whole process.

How much does this rigmarole save us? If we have $\decay =  0.98$ we will have
to store $0.029$ bits on average. Compared to storing a new 32-bit integer or
floating-point number, this reduces memory requirements by a factor of one
thousand.
%
\begin{algorithm}
...
\end{algorithm}
%
Note that we could have used an arithmetic coding scheme for our information
buffer~\citep{steinruecken2014a}. An entirely different approach to the question
of memory use with RMD is checkpointing. This is sometimes used even for
differentiation of neural net loss functions with resepct to {\primal}
parameters~\citep{martens2012training}. The idea is to store just a small number
of parameter vectors during training, and recompute sections of the training
...
thousands of minibatches.  Luckily, when the decay factor is close to 1, the
amount of information lost at each iteration is small.
...
\newcommand{\omniimage}[1]{\multirow{2}{4cm}{\includegraphics[width=3.7cm, height=1.4cm, angle=0]{../experiments/Feb_4_augmented_omniglot/2_rotated_90/learned_corr_#1.pdf}}}%
...
\begin{tabular}{cccc}%
                  ...
                  \end{tabular}}%
                    & Train & Test\\
                  & & error & error \\ \\
Separate & \omniimage{0}%
%\begin{tabular}{c}\omniimage{0}\\\omniimage{1}\\\omniimage{2}\end{tabular}}%
 & & \\ 
weights  & &   0.61 & 1.34 \\ \\
Tied     & \omniimage{1} & & \\
weights  & &   0.90 & 1.25 \\ \\
Learned  & \omniimage{2} & & \\
sharing  & &   0.60 & \bf 1.13
\end{tabular}
...
Figure \ref{fig:chaos} illustrates phenomenon when training a neural network having 2 hidden layers.
%
\begin{figure}[t]
\vskip 0.2in
\begin{center}
