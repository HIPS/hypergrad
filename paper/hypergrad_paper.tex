...
\begin{algorithmic}[1]
   ...
   \State $i = i \times d$ \Comment{make room for new digit}
   \State $i = i + (c \mod d)$ \Comment{store digit lost from division}
   \State $c = c \div d$ \Comment{divide by denominator}
   \State $c = c \times n$ \Comment{multiply by numerator}
   \State $b = i \mod n$ \Comment{extract digit from buffer}
   \State $i = i \div n$ \Comment{shorten information buffer}
   \State $c = c + b$ \Comment{append digit to value}
