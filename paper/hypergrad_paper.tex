...
We illustrate this ``architecture learning'' with a multitask learning problem,
the Omniglot data set \citep{Omniglot}. This data set consists of 28 by 28 pixel
greyscale images of characters from 50 alphabets with up to 55 characters in
...
Rather than learning the penalty matrix directly (which for stability is
constrained to be positive semi-definite) our hyperparameters are three
ten-by-ten transformations from a space of ``whitened'' weights, whose penalty
matrix is the identity matrix, to the space of weights used in the actual neural
networks. This is an unidentifiable but unconstrained parameterization. Each
alphabet's neural net had hidden layers of size 784, 400 and 200.
