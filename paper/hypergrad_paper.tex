...
This section shows some proof-of-concept experiments in which we can more richly parameterize our training and regularization schemes in ways that would have been previously impractical to optimize.



\subsection{Gradient-based optimization of gradient-based optimization}

Modern neural net training procedures often employ various heuristics to set learning rate schedules, or set their shape using one or two hyperparameters set by cross-validation \cite{lecun1989backpropagation}, \citep{dahl2014multi}, \citep{sutskever2013importance}.
...
To more directly shed light on good learning rate schedules, we jointly optimized separate learning rates for \emph{every single learning iteration} of training of a deep neural network.
...
To avoid learning an optimization schedule that depended on the quirks of a particular random initialization, we evaluation of hyper-gradients used a different random seed.
These random seeds were used both to initialize network weights and to choose mini batches.

We optimized the learning rate schedules on the training

Because learning schedules can implicitly regularize networks~\ref{erhan2010does}, in early stopping, 
%
\begin{figure}[h!]
\vskip 0.2in
\begin{center}
...
We also optimized a separate weight initialization scale hyperparameter for each type of parameter (weights and biases) in each layer.
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=\columnwidth]{../experiments/Feb_4_augmented_omniglot/2_rotated_90/all_alphabets.png}
...
\begin{center}
\begin{tabular}{ccc}
Independent & Full & Learned \\
\hspace{-1em}\includegraphics[width=0.3 \columnwidth]{../experiments/Feb_4_augmented_omniglot/2_rotated_90/learned_corr_0.pdf} &
\hspace{-1em}\includegraphics[width=0.3 \columnwidth]{../experiments/Feb_4_augmented_omniglot/2_rotated_90/learned_corr_1.pdf} &
\hspace{-1em}\includegraphics[width=0.3 \columnwidth]{../experiments/Feb_4_augmented_omniglot/2_rotated_90/learned_corr_2.pdf}
\end{tabular}
...
\begin{center}
\begin{tabular}{ccc}
\input{../experiments/Feb_4_augmented_omniglot/2_rotated_90/results_table.tex}
\end{tabular}
\caption{Training and test error on omniglot dataset.}
