...
recurrent neural nets, multitask neural nets. We can think of these
architectures as hard constraints of weight sharing or weight absence on an
otherwise fully-connected neural network. By softening these hard architetural
...
problem, the Omniglot data set \citep{Omniglot}. This data set consists of 28 by 28
...
separate neural net for each alphabet, one multitask approach would be for all the
neural nets to share a single first layer, pooling statistical
strength to learn generic Gabor-like filters, with separate higher layers specific to
each alphabet.

We can parameterize any architecture based on weight sharing or weight absence
with a pairwise quadratic penalty on the weights, $\vw^T A \vw$, where $A$ is a
number-of-weights by number-of-weights matrix. Learning this enormous matrix $A$
is clearly infeasible but we can implicitly build such a matrix from lower
dimensional structures of manageable size.

For the Omniglot problem, we learn a penalty for each alphabet pair and neural
net layer separately. Thus, for three-layer neural networks and ten alphabets,
...
learning system is able to pick up on high level similarities as well as
low-level similarities, we repeat these five alphabets, with the images rotated
by 90 degrees (Figure \ref{fig:omniglot_images}).

Rather than learning the penalty matrix directly (which is constrained to be
positive semi-definite) our hyperparameters are three ten-by-ten transformations
from a space of ``whitened'' weights, whose penalty matrix is the identity
matrix, to the space of weights used in the actual neural networks. This is an
unidentifiable but unconstrained parameterization.
