...
We experimented with using several standard stochastic optimization methods for meta-optimization, including plain SGD, RMSprop [cite], and minibatch conjugate gradients.
The results in this section used Adam~\citep{Adam14}, a variant of RMSprop that includes momentum terms.
...
We choose this model because every weight corresponds to an input-pixel and output-label pair, meaning that these parameters might be more interpretable.
%
\begin{figure}[h!]
\begin{center}
...
\newcommand{\omniimage}[1]{\multirow{2}{*}{\hspace{-4mm}{{\includegraphics[width=0.55\columnwidth, angle=0]{../experiments/Feb_4_augmented_omniglot/2_rotated_90/learned_corr_#1.pdf}}\hspace{-4mm}}}}
...
As shown in Figure \ref{fig:nn weight init scales}, the meta-optimization in this experiments did not completely converge after 50 meta-iterations.
...
Reversible Learning might allow memory-efficient \primal{} gradient computations with respect to very long computations performed by either architecture.
