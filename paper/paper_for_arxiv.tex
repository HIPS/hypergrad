...
\hspace{-.1in}\includegraphics[width=0.878\columnwidth, clip, trim=2cm 0cm 0cm 0cm]{fig_1.pdf}
\end{tabular}
...
%every $\sqrt{T}$th training step (for example), and recomputes the missing steps of the training procedure (forwards) as needed during the backward pass.
%The total memory requirement is then $\mathcal{O}(\sqrt{T}N)$ and the time complexity is $\mathcal{O}{T}$.
%For very long computations, checkpointing to reduce 
...
\hspace{-1em}\includegraphics[width=0.5\columnwidth, height=0.5\columnwidth]{fig_3.pdf} &
\hspace{-1em}\includegraphics[width=0.5\columnwidth, height=0.5\columnwidth]{fig_4.pdf}
\end{tabular}
\vskip -0.1in
...
\begin{tabular}{cc}
 Biases & Weights \\
\hspace{-1em}\includegraphics[width=0.5\columnwidth, height=0.5\columnwidth]{fig_6.pdf} &
\hspace{-1em}\includegraphics[width=0.5\columnwidth, height=0.5\columnwidth]{fig_7.pdf}
\end{tabular}
...
% (scaled so that rows and columns sum to one)
...
\hspace{-3mm}\includegraphics[width=0.85\columnwidth]{all_alphabets.png}
\end{tabular}
...
%
\newcommand{\omniimagea}[2]{\parbox{4em}{\includegraphics[width=1.4cm]{minifigs/learned_corr_#1_#2.pdf}}}%
...
%\vskip 0.2in
\begin{center}
\includegraphics[width=0.9\columnwidth]{fig_11.pdf}
\vskip -0.1in
...
For example, Gaussian-process-based optimization methods %~\cite{snoek2012practical}
...
%Checkpointing is sometimes used for differentiation of neural network loss functions with respect to {\primal} parameters~\citep{martens2012training}. 
...
Exactly reversing these dynamics might allow more memory-efficient \primal{} gradient evaluations of their outputs on very long input sequences. % of the very long forward computations performed by these architectures.
